...
def plot_heatmap(folder,params,fcolor):
    ...
    c_list = [sublist[poi]*2*np.pi/(const_val*1000) for sublist in converged_points] # if sublist[poi]]
    ...
    # HERE
    levels = np.geomspace(0.4,10, 45 + 1) # levels=levels # angle - np.linspace(65,91,53) , dV = np.linspace(0,15, 45 + 1) / np.geomspace(0.4,10, 45 + 1)
    norm = mpl.colors.LogNorm(vmin=0.4,vmax=10)
    #print(min(c_list),max(c_list))
    hb = plt.tricontourf(x_conv,y_conv,c_list,levels=levels,norm=norm,cmap='plasma',zorder=1) # vmin=min(c_list),vmax=max(c_list) ,  dV -> norm=norm
...
if __name__ == "__main__":
    ...
    folder_parent = "/home/lha43/Desktop/Lucas/pareto_points_2025"
    ...
    folder = f"{folder_parent}/Nov_16_2025_GREAT_z"
    ...
    cbar.set_label('Cone angles $\phi$ [deg]') # '$\Delta V$ [km/s]' # 'Cone angles $\phi$ [deg]'
