...
if __name__ == "__main__":
    ...
    # #graph(hover_eq,np.linspace(0.01,0.5,1000),'#D81B60') # #965081

    # #plt.subplot(2,2,1)
    # plt.sca(ax00)
    # fcolor = '#000000' # '#000000' #'#D81B60'
    # const_val = 0.05
    # params = [const,const_val,plot_rand,poi]
    ...
    # Traj_data = [converged_m,Umax,period,target_z,TrajI_m,theta,min_alt,dV_m,-time_cpu_m,rand_flag,f_m]
    #             [converged_m,Umax,period,target_z,TrajI_m,theta,min_alt,dV_m,time_cpu_m,rand_flag,f_m]
    const = '$P$' # $P$ or $z_0$
    const_val = 2 # period value  
    plot_rand = 0 # whether or not to plot random points
    poi = 5 # Theta
    params = [const,const_val,plot_rand,poi]
    ...
    umax_search_min, umax_search_max = 1.55, 1.65
    z_search_min, z_search_max = 0.16, 0.17 

    for i in range(len(pareto)):

        if pareto[i][1] >  umax_search_min and  pareto[i][1] < umax_search_max and \
           pareto[i][3] > z_search_min and pareto[i][3] < z_search_max:
        #if (i % 6) != 0:
            if pareto[i][0]:
                Traj = pareto[i][4]
                cone_ang = pareto[i][5]
