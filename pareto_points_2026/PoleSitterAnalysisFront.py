...
def plot_full_front(points_conv,const_val,color):
    ...
    plt.plot(front[0],front[1],c=color,marker='o',ms=1.5,lw=1.5, label = f'Front, $P$ = {const_val} [TU]')
...
def plot_search(folder,params,fcolor):
    ...
    # draw utopia points
    plt.scatter(min(x_conv),min(y_conv),c=fcolor,marker='x',s=100,linewidths=3,label=f'Utopia point, $P$ = {const_val} [TU]')
    print(min(x_conv),min(y_conv))
