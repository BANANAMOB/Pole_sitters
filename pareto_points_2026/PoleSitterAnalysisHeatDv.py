...
def plot_heatmap(folder,params,fcolor):
    ...
    # HERE 
    levels = np.geomspace(1,16, 45 + 1) # levels=levels # angle - np.linspace(65,91,53) , dV = np.linspace(0,15, 45 + 1) / np.geomspace(0.4,10, 45 + 1)
    norm = mpl.colors.LogNorm(vmin=1,vmax=16)
    #print(min(c_list),max(c_list))
    hb = plt.tricontourf(x_conv,y_conv,c_list,norm=norm,levels=levels,cmap='plasma',zorder=1) # vmin=min(c_list),vmax=max(c_list),levels=levels,  dV -> norm=norm
...
if __name__ == "__main__":
    ...
    cbar.set_label('$\Delta V_{SK}$ [km/s]') # '$\Delta V$ [km/s]' # 'Cone angles $\phi$ [deg]'
