...
def find_traj(target_x,target_y,target_z,period,Umax,IG):
    ...
    try: 
        ...
        if f:
            ...
            converged = 1
            Traj_data = [converged,Umax,period,target_z,TrajI,theta]
        else:
            converged = 0
...
def wifi_sweep(pareto,target_z,first_z,Umax,period,wifi_density,wifi_bars):
    base_z = target_z
    base_Umax = Umax
    angles = np.linspace(0,-np.pi,wifi_density)
    bars = np.linspace(0,target_z-first_z,wifi_bars)
    for angle in angles:
...
# def pareto_2D_bomb():
# def pareto_2D_fine_search():
# Umax hover point
# plot norm (to represent altitude)
...
def pareto_2D_coarse_search(s_params, opt_params,n_save):
    ...
    for p_y in y_range:
        ...
        while max_non_conv_clu > not_conv_clu:
            ...
            Traj_data, converged = find_traj(target_x,target_y,target_z,period,Umax,IG) 
            pareto.append(Traj_data)
            ...
            if n_point % n_rand == 0:
                target_z = c if (decision_params[0] == 2) else random.uniform(min(y_range),max(y_range)) if (decision_params[0] == 1) else random.uniform(0.0001,max_x)
                Umax = c if (decision_params[1] == 2) else random.uniform(min(y_range),max(y_range)) if (decision_params[1] == 1) else random.uniform(0.0001,max_x)
                period = c if (decision_params[2] == 2) else random.uniform(min(y_range),max(y_range)) if (decision_params[2] == 1) else random.uniform(0.0001,max_x)
                Traj_random, _ = find_traj(target_x,target_y,target_z,period,Umax,IG) 
...
if __name__ == "__main__":
    ...
    # Search parameters:
    ls = 0.05 # small step size multiplier 
    ds = 0.001 # large step size multiplier 
    ...
    target_z = 0.3 # 0.5 altitude at the start of the orbit 
    Umax = np.linspace(0.1,1.9,25) # Maximum control authority
    period = 2.085034838884136 # 2.085034838884136 # Period
    ...
    n_save = 10 # Save data for every n_save points

    s_params = [ls,ds,max_non_conv,max_non_conv_clu, n_rand] # preserve order
