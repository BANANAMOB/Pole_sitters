...
def pareto_wifi_search():
    ...
    # pareto wifi search
    n = 10000 # loop limit
    ds = 1/1000 # small step size multiplier 
    ...
    for j in range(n):
        ...
        if target_z < 0:
            target_z = last_conv
            k = round (( 1 - (target_z/(max_target_z-0.3215)) )/ds )
            conv_num = 0
        ...
        if converged: 
            ...
            if conv_num > conv_num_max:
                ...
                target_z = (max_target_z-0.3215)*((1-(ds*k))-s*s_mult)
                k = round (( 1 - (target_z/(max_target_z-0.3215)) )/ds )
            else:
                target_z = (max_target_z-0.3215)*(1-(ds*k))
                k = k + 1
        else: 
