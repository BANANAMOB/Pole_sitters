...
if __name__ == "__main__":
    ...
    # NORM
    min_acc_z = min(o[3] for o in filtered_orbits)
    max_acc_z = max(o[3] for o in filtered_orbits)
    norm = mcolors.Normalize(vmin=min_acc_z, vmax=max_acc_z)
    cmap = cm.plasma
    print(f"Acceleration range: {min_acc_z:.3f} to {max_acc_z:.3f}")
    ...
    plt.colorbar(cm.ScalarMappable(cmap=cmap, norm=norm),
                 cax=cbar_ax, orientation='horizontal', label='$u_{max}$ [DU/TU$^2$]')
